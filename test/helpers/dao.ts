...
export const newDaoFactory = async (): Promise<DAOFactory> => {
  ...
  return (await DAOFactory.deploy(kernelBase.address, aclBase.address, registryFactory.address)) as DAOFactory
}
...
export const newDao = async (root: SignerWithAddress): Promise<[Kernel, ACL]> => {
  ...
  const daoAddress = await getEventArgument(daoFactory, daoReceipt.transactionHash, "DeployDAO", "dao")

  const kernel = (await ethers.getContractAt("Kernel", daoAddress, root)) as Kernel
