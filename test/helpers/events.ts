...
export const getEventArgument = async (contract: Contract, txHash: string, eventName: string, eventArg: string): Promise<any> => {
  ...
  const [event] = events.filter(event => event.transactionHash === txHash && event.topics[0] === filter.topics[0])
